...
@torch.no_grad()
def evaluation_vae(out_dir, val_loader, net, writer, ep, best_mse, eval_wrapper, save=True, draw=True):
    net.eval()
    ...
    for batch in val_loader:
        word_embeddings, pos_one_hots, caption, sent_len, MB_motion, m_length, token = batch

        MB_motion = MB_motion.cuda()
        m_length = m_length.cuda()

        pred_pose_eval, loss_dict = net(MB_motion)

        # mask: [bs, seq], True 表示无效帧
        mask = torch.arange(MB_motion.shape[1]).unsqueeze(0).expand(MB_motion.shape[0], -1).cuda() >= m_length.unsqueeze(1)
        pred_pose_eval = pred_pose_eval.masked_fill(mask.unsqueeze(-1).unsqueeze(-1), 0)

        valid_mask = (~mask).unsqueeze(-1).unsqueeze(-1)  # [bs, seq, 1, 1]

        # sum-reduction + 手动归一化，避免 padding 干扰
        mse_loss = F.mse_loss(pred_pose_eval * valid_mask, MB_motion * valid_mask, reduction="sum")
        num_valid = valid_mask.sum()

        if num_valid > 0:
            total_loss += mse_loss.item()
            total_valid_elements += num_valid

    # 最终平均 MSE（按有效元素）
    avg_mse = total_loss / max(total_valid_elements, 1)

    msg = f"--> \t Eva. Ep {ep}: Avg MSE Loss = {avg_mse:.6f}"
    print(msg)

    if draw:
        writer.add_scalar('Test/MSE_Loss', avg_mse, ep)

    if avg_mse < best_mse:
        msg = f"--> --> \t MSE Loss Improved from {best_mse:.6f} to {avg_mse:.6f} !!!"
        if draw: print(msg)
        best_mse = avg_mse
        if save:
            torch.save({'vae': net.state_dict(), 'epoch': ep}, os.path.join(out_dir, 'net_best_mse.tar'))

    net.train()
    return best_mse, writer
